...
import type { Session, SessionFull, ToolCall } from "./types.ts";
...
export class TelemetryWriter {
	...
	async readSessions(): Promise<Array<Session | SessionFull>> {
		...
		return lines.map((line: string) => JSON.parse(line) as Session | SessionFull);
	}
	...
	async readToolCalls(): Promise<ToolCall[]> {
		...
		return lines.map((line: string) => JSON.parse(line) as ToolCall);
	}
	...
	async getStats(): Promise<{
		...
		for (const session of sessions) {
			...
			toolCallCount += session.toolCalls.reduce(
				(sum: number, tc: { callCount: number }) => sum + tc.callCount,
				0,
			);
	...
	async listExports(): Promise<string[]> {
		...
		return files.filter((f: string) => f.endsWith(".json") || f.endsWith(".jsonl"));
	}
}
